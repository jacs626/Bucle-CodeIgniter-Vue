...
const handleSubmit = async (data: Partial<Block>) => {
  try {
    if (editingBlock.value) {
      await blocksStore.updateBlock(editingBlock.value.id, data)
  ...
  <div class="max-w-6xl mx-auto space-y-6">
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-slate-500 dark:text-slate-400 text-sm">Gestiona todos tus bloques y tareas</p>
      </div>
      <button 
        @click="openCreate"
    ...
    <div v-else-if="filteredBlocks.length === 0" class="text-center py-12">
      ...
      <p class="text-slate-500 dark:text-slate-400">No hay bloques {{ filter !== 'all' ? filter === 'pending' ? 'pendientes' : 'completados' : '' }}</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <p v-if="block.data?.description" class="text-sm text-slate-500 dark:text-slate-400 mb-3 line-clamp-2">
          {{ block.data.description }}
        </p>

        <div v-if="block.schedule" class="mb-3 text-xs text-slate-500 dark:text-slate-400">
          📅 {{ block.schedule.type === 'fixed' ? block.schedule.date : block.schedule.type === 'interval' ? `Cada ${block.schedule.intervalHours}h` : 'Semanal' }}
        </div>

        <div class="flex items-center justify-between">
          <StatusBadge :status="block.status === 'done' ? 'done' : 'pending'" size="sm" />
          <span v-if="block.order_index" class="text-xs text-slate-400">#{{ block.order_index }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden">
        <BlockForm
          ...
          @close="showForm = false; editingBlock = null"
          @submit="handleSubmit"
        />
      </div>
    </div>
